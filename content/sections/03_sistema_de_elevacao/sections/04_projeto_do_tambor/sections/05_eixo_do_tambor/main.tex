\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{content/sections/03_sistema_de_elevacao/sections/04_projeto_do_tambor/sections/05_eixo_do_tambor/drawings/eixo_tambor.pdf}
  \caption{Representação do eixo do tambor (Autor)}
  \label{fig:eixo_tambor}
\end{figure}
...
Com isso, é possível obter a equação para calcular o diâmetro mínimo do eixo do tambor:
...
Com base no diâmetro mínimo calculado, é possível definir as dimensões representadas na Figura \ref{fig:eixo_tambor}.

\begin{equation}
  d_1 = \num{1.2} \cdot d_{\text{eixo}}
  \label{eq:d_1_eixo_do_tambor}
\end{equation}

\begin{equation}
  d_e = d_1 - 20 \left[ \si{\milli\meter} \right]
  \label{eq:d_e_eixo_do_tambor}
\end{equation}

\begin{equation}
  d = d_e - 10 \left[ \si{\milli\meter} \right]
  \label{eq:d_eixo_do_tambor}
\end{equation}

Para o raio $r$, utilizou-se \SI{1.5}{\milli\meter}.

Substituindo os valores na equação \eqref{eq:d_1_eixo_do_tambor}:
\begin{align*}
  d_1 & = \num{1.2} \cdot \num{73.71}         \\
  d_1 & = \SI{88.45}{\milli\meter}            \\
  d_1 & \approx \boxed{\SI{90}{\milli\meter}}
\end{align*}

Substituindo os valores na equação \eqref{eq:d_e_eixo_do_tambor}:
\begin{align*}
  d_e & = \SI{90}{\milli\meter} - 20    \\
  d_e & = \boxed{\SI{70}{\milli\meter}}
\end{align*}
